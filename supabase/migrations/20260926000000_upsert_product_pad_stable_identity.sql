-- Stable identity for Submit Pad drafts.
--
-- Previously upsert_product_pad matched rows by (maker_id, slug) only, so
-- renaming the address after a draft save forked a second row and orphaned
-- the first — burning slugs and stranding ghost drafts.
--
-- The payload may now carry the draft id. When present, valid, and owned by
-- the caller, the row is updated in place — including renames, guarded by the
-- global products_slug_key unique constraint with a friendly error the client
-- already maps to "That address is taken". Payloads without an id behave
-- exactly as before (maker + slug match, else insert).
...
begin
  ...
  -- Stable identity: an owned id always wins over the slug match, so renames
  -- update the same row instead of forking a new one.
  begin
