	...
	<div class="grid gap-6 pb-10 lg:grid-cols-[minmax(0,1fr)_300px]">
		<section class="space-y-3" aria-label="Problem records">
			{#each filtered as problem (problem.id)}
				<a href={problem.workspacePath} class="group block rounded-[18px] bg-[var(--pc-surface-2)] p-4 transition-[background-color,transform] duration-150 hover:bg-[var(--pc-surface)] active:scale-[0.99] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--pc-accent-light)]">
					<div class="flex items-start gap-3"><span class="grid size-10 shrink-0 place-items-center rounded-[11px] bg-[var(--pc-surface)] text-[var(--pc-text-muted)]"><Compass size={17} weight="Outline" aria-hidden="true" /></span><span class="min-w-0 flex-1"><span class="flex flex-wrap items-center gap-2"><strong class="text-[14px] font-medium group-hover:text-[var(--pc-accent-light)]">{problem.title}</strong><Chip size="xs">{problem.status}</Chip></span><span class="mt-1 block text-xs text-[var(--pc-text-muted)] opacity-70">{problem.productName} · {problem.productArea} · {problem.owner}</span><span class="mt-2 block text-sm leading-relaxed text-[var(--pc-text-muted)] opacity-80">{problem.statement}</span></span></div>
		...
		<aside class="space-y-4">
			<Card padding="md"><div class="flex items-center gap-2"><Compass size={15} weight="Outline" class="opacity-55" aria-hidden="true" /><h2 class="text-[13px] font-medium">Why keep problems separate?</h2></div><p class="mt-3 text-xs leading-relaxed text-[var(--pc-text-muted)] opacity-75">Feedback often asks for a fix. A problem records the need behind that request, so the team can compare more than one solution.</p><a href="/workspace/inbox" class="mt-4 inline-flex items-center gap-1 text-xs text-[var(--pc-accent-light)]">Find the original messages <ArrowRight size={13} weight="Outline" aria-hidden="true" /></a></Card>
			<Card padding="md"><p class="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--pc-accent-light)]">Problem states</p><div class="mt-4 space-y-3 text-xs"><div><p class="font-medium">Needs context</p><p class="mt-1 text-[var(--pc-text-muted)] opacity-70">We need more information before choosing a direction.</p></div><div><p class="font-medium">Ready for decision</p><p class="mt-1 text-[var(--pc-text-muted)] opacity-70">The need is clear enough to compare options.</p></div><div><p class="font-medium">Planned</p><p class="mt-1 text-[var(--pc-text-muted)] opacity-70">A product decision is connected to upcoming work.</p></div></div></Card>
		</aside>
	</div>
</div>
