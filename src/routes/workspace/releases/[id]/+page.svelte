...
{#if release}
	<div class="mx-auto w-full max-w-[920px] px-4 sm:px-6">
		...
		<div class="grid gap-6 pb-10 lg:grid-cols-[minmax(0,1fr)_280px]">
			...
			<aside class="space-y-4">{#if product}<div class="min-h-[200px] py-4" aria-hidden="true"></div>{/if}{#if release.githubReleaseUrl}<Card padding="md"><div class="flex items-center gap-2"><Rocket size={15} weight="Outline" class="opacity-55" /><h2 class="text-[13px] font-medium">GitHub release</h2></div><a href={release.githubReleaseUrl} target="_blank" rel="noreferrer" class="mt-3 inline-flex items-center gap-1 text-xs text-[var(--pc-accent-light)] hover:underline">Open release ↗</a></Card>{/if}<Card padding="md"><div class="flex items-center gap-2"><Book size={15} weight="Outline" class="opacity-55" /><h2 class="text-[13px] font-medium">Help docs</h2></div>{#each relatedDocs as doc (doc.slug)}<a href={doc.publicPath} class="mt-3 block text-xs text-[var(--pc-accent-light)] hover:underline">{doc.title}</a>{/each}</Card></aside>
		</div>
	</div>
{:else}
