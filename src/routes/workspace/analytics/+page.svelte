...
<div class="mx-auto w-full max-w-[1180px] px-4 sm:px-6">
	<WorkspaceHeader title="Analytics" description="See if people can find answers, understand updates, and know what happens next." />
	...
	<section class="pb-6" aria-labelledby="loop-title"><div class="mb-3 flex items-end justify-between gap-3"><div><h2 id="loop-title" class="mt-1 text-lg font-medium">See where each update came from</h2></div><span class="text-xs text-[var(--pc-text-faint)]">Open any list to learn more</span></div><div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{#each loopSteps as step, index}<a href={step.href} class="group rounded-[16px] bg-[var(--pc-surface-2)] p-4 transition-[background-color,transform] duration-150 hover:bg-[var(--pc-surface)] active:scale-[0.99]"><div class="flex items-center justify-between gap-2"><span class="grid size-7 place-items-center rounded-[9px] bg-[var(--pc-surface)] text-[11px] font-medium text-[var(--pc-text-muted)]">{index + 1}</span><ArrowRight size={14} weight="Outline" class="opacity-40 transition-transform duration-150 group-hover:translate-x-0.5" aria-hidden="true" /></div><p class="mt-4 text-sm font-medium">{step.label}</p><p class="mt-1 text-[11px] text-[var(--pc-text-muted)] opacity-70">{step.detail}</p><p class="mt-3 text-2xl font-medium tabular-nums tracking-tight">{step.value}</p></a>{/each}</div></section>
	<div class="grid gap-6 pb-10 lg:grid-cols-[minmax(0,1fr)_300px]">
		<Card padding="lg"><div class="flex items-center justify-between gap-3"><div><h2 class="mt-1 text-lg font-medium">Products by public reads</h2></div><TrendUp size={17} weight="Outline" class="opacity-55" /></div><div class="mt-6 space-y-4">{#each productsByReads as product (product.slug)}<a href={product.publicPath} class="block"><div class="flex items-center gap-3"><img src={product.avatar} alt="" class="size-8 rounded-[9px] object-cover" /><span class="min-w-0 flex-1 truncate text-xs font-medium">{product.name}</span><span class="text-xs tabular-nums text-[var(--pc-text-muted)]">{product.reads}</span></div><div class="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--pc-surface)]"><div class="h-full rounded-full bg-[var(--pc-accent)]" style:width={`${product.share}%`}></div></div></a>{/each}</div></Card>
