...
<div class="w-full">
	...
	<div class="px-3 md:px-4">
		...
		{#if tab === 'videos'}
			<div class="py-8 grid gap-x-4 gap-y-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 pc-enter-stagger">
				{#each filtered as item (item.id)}
					<VideoCard {item} />
				{/each}
			</div>
		{:else if tab === 'changelog'}
			<div class="py-6 space-y-3 max-w-[880px] pc-enter-stagger">
				{#each filtered.filter((s) => s.type === 'changelog' || s.type === 'launch') as item}
					<div class="pc-bezel p-1.5">
						<div class="pc-bezel-inner p-4 flex gap-4 bg-[var(--pc-surface)]">
							<img src={item.thumbnail} alt="" class="hidden md:block w-[160px] aspect-video rounded-[10px] object-cover shrink-0 border border-[var(--pc-border)]" />
							<div class="min-w-0">
								<p class="text-xs font-700 tracking-wide uppercase text-[var(--color-blue-600)]">{item.type} • {item.postedAt}</p>
								<h3 class="mt-1 text-[16px] font-700 leading-tight tracking-tight">{item.title}</h3>
								<p class="mt-1 text-sm text-[var(--pc-text-muted)] line-clamp-2 text-pretty">{item.description}</p>
								<a href="/watch/{item.id}" class="mt-3 inline-flex pc-btn-primary text-xs px-4 py-1.5">View update <span class="pc-btn-icon">→</span></a>
							</div>
						</div>
					</div>
				{/each}
		...
		{:else}
			<div class="py-6 space-y-3 max-w-[880px] pc-enter">
				<div class="pc-bezel p-1.5">
					<div class="pc-bezel-inner p-4 flex gap-3 bg-[var(--pc-surface)]">
				...
				{#each filtered.filter((s) => s.type === 'incident' || s.type === 'fix') as item}
					<div class="pc-bezel p-1.5">
						<div class="pc-bezel-inner p-4 bg-[var(--pc-surface)]">
							...
							<h3 class="font-700 tracking-tight">{item.title}</h3>
							<p class="text-sm text-[var(--pc-text-muted)] text-pretty">{item.description}</p>
						</div>
					</div>
				{/each}
				{#if filtered.filter((s) => s.type === 'incident' || s.type === 'fix').length === 0}
					<div class="pc-bezel p-1.5">
						<div class="pc-bezel-inner p-6 text-center bg-[var(--pc-surface)]">
							<p class="text-sm font-700">No incidents recently</p>
							<p class="text-xs text-[var(--pc-text-muted)]">Green across the board — subscribe for instant incident pushes.</p>
						</div>
					</div>
				{/if}
			</div>
