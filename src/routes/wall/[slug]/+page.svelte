...
{#if product}
	<div class="mx-auto w-full max-w-[1060px] px-4 sm:px-6">
		<header class="pb-8 pt-8 sm:pt-12"><a href={product.publicPath} class="inline-flex items-center gap-1 text-xs text-[var(--pc-text-muted)] hover:text-[var(--pc-text)]"><ArrowLeft size={13} weight="Outline" /> {product.name}</a><div class="mt-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 class="mt-2 text-[28px] font-medium tracking-tight md:text-[40px]">Why teams choose {product.name}</h1><p class="mt-2 max-w-[58ch] text-[14px] md:text-[15px] leading-[1.65] tracking-[-0.009em] text-[var(--pc-text-muted)] max-w-[60ch] text-pretty antialiased">Real words from people who use this product at work.</p></div><Button href="/feedback/new" variant="primary" size="md"><Message size={15} weight="Outline" /> Share your experience</Button></div></header>
