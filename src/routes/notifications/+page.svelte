...
<div class="w-full max-w-[883px] mx-auto px-6 max-sm:px-4">
	...
	<header class="pt-10 pb-5 max-sm:pt-8 max-sm:pb-4">
		<div class="flex items-baseline justify-between gap-4">
			...
			{#if unreadCount > 0}
				<button onclick={markAllRead} class="inline-flex items-center rounded-full bg-[var(--pc-surface-2)] px-3 py-1.5 text-[13px] text-[var(--pc-accent)] hover:bg-[var(--pc-surface)] transition-colors">
					Mark all read
				</button>
			{/if}
		</div>
		<p class="mt-2 text-[13px] text-[var(--pc-text-muted)] opacity-40">
	...
	<div class="flex items-center gap-1 pb-4">
		...
		>
			Unread {#if unreadCount > 0}<span class="text-[11px] opacity-40">{unreadCount}</span>{/if}
		</button>
	</div>
				...
				<div class={[
					...
					<div class="min-w-0 flex-1">
						<div class="flex items-center gap-2">
							<img src={n.productAvatar} alt="" class="size-4 rounded object-contain" />
						...
						<span class="mt-1 block text-[11px] text-[var(--pc-text-faint)] opacity-35">{n.time}</span>
					</div>
				</div>
			{/each}
		</div>
	{:else}
		<div class="py-16 flex flex-col items-center text-center pc-enter">
			<div class="mx-auto size-10 rounded-full bg-[var(--pc-surface-2)] grid place-items-center">
				<Bell size={16} weight="Outline" class="opacity-30" />
			</div>
			<p class="mt-3 text-sm font-medium">No unread notifications</p>
			<p class="mt-1 text-[13px] text-[var(--pc-text-muted)] opacity-40">You're all caught up.</p>
		</div>
	{/if}
</div>
