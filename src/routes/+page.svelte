...
<!-- Single 883px container — everything aligned -->
<div class="w-full max-w-[883px] mx-auto px-6 max-sm:px-4">

	<!-- ─── Header ─── -->
	<!-- Math: title baseline at ~60% of header height, tagline sits below with 12px gap -->
	<header class="pt-10 pb-6 max-sm:pt-8 max-sm:pb-4">
		<!-- Row 1: Title + Live badge — same baseline -->
		<div class="flex items-baseline gap-3">
			<h1 class="text-[22px] md:text-[26px] font-medium leading-none tracking-tight">
				Top product launches this week
			</h1>
			<span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-normal" style:background="rgba(119, 152, 18, 0.15)" style:color="rgb(119, 152, 18)">Live</span>
		</div>

		<!-- Row 2: Metadata — clean, single-line, with icon accent -->
		<div class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-[13px] text-[var(--pc-text-muted)]">
			...
			<span class="text-[var(--pc-text-faint)] opacity-30">·</span>
			...
			<span class="hidden md:inline text-[var(--pc-text-faint)] opacity-30">·</span>
			<span class="hidden md:inline opacity-70">SEO-ready pages · AI-readable listings</span>
		</div>

		<!-- Row 3: Tagline — slightly indented from title for hierarchy -->
		<p class="mt-2 text-[13px] leading-relaxed text-[var(--pc-text-muted)] max-w-[52ch] text-pretty opacity-40">
			Launch your tool, get found on Google and in ChatGPT answers.
		</p>
	</header>

	<!-- ─── Filter bar ─── -->
	<!-- Sticky within the 883px container — no negative margin hacks -->
	<nav class="sticky top-[var(--pc-header-h)] z-20 -mx-6 px-6 max-sm:-mx-4 max-sm:px-4 pb-3 bg-[var(--pc-bg)]/90 backdrop-blur" aria-label="Launch filters">
		<!-- Bottom separator — 1px, full width of container -->
		...
		<div class="flex items-center gap-2 overflow-x-auto scrollbar-none pt-1">
			...
			<span class="ml-auto hidden md:inline-flex items-center gap-1.5 text-xs text-[var(--pc-text-muted)] opacity-40 shrink-0">
				<Flame size={12} weight="Outline" />
				{filtered.length}
			</span>
		</div>

		<!-- Mobile search — inline, no border -->
		<div class="mt-2.5 flex md:hidden items-center gap-2 rounded-full bg-[var(--pc-surface-2)] px-3 py-2">
			<Search size={14} weight="Outline" class="opacity-40" />
	...
	<div class="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-8 pt-5">
		...
		<!-- Feed column -->
		<main class="min-w-0">
			<!-- Desktop search — left-aligned, no border, pill shape -->
			<div class="hidden md:flex items-center gap-3 mb-5">
				...
				<span class="text-xs text-[var(--pc-text-faint)] opacity-30 shrink-0">{filtered.length} of {week.products}</span>
			</div>

			{#if filtered.length === 0}
				<!-- Empty state — centered, spacious -->
				<div class="rounded-[30px] bg-[var(--pc-surface-2)] px-6 py-16 flex flex-col items-center text-center pc-enter">
					<div class="mx-auto size-10 rounded-full bg-[var(--pc-surface)] grid place-items-center">
						<Search size={16} weight="Outline" class="opacity-40" />
					</div>
					<p class="mt-4 text-sm font-medium">No launches match</p>
					<p class="mt-1 text-xs text-[var(--pc-text-muted)] opacity-40">Try a different filter or search term.</p>
					<button onclick={() => { active = 'all'; q = ''; }} class="pc-btn-primary mt-5">Clear filters</button>
				</div>
			{:else}
				<!-- Launch cards — tight vertical rhythm -->
				...
				<!-- Load more — centered, minimal -->
				<div class="mt-8 flex flex-col items-center gap-2.5">
					<p class="text-xs text-[var(--pc-text-faint)] opacity-30">
						{week.live - filtered.length} more this week
					</p>
					<button class="pc-btn-ghost">Load more</button>
				</div>
			{/if}
		</main>
		...
		<aside class="hidden lg:block space-y-4 h-fit sticky top-[calc(var(--pc-header-h)+60px)]">
			<!-- Stats card -->
			<div class="rounded-[20px] bg-[var(--pc-surface-2)] p-4">
				...
				<div class="mt-3 grid grid-cols-3 gap-1.5">
					{#each [
						...
						<div class="rounded-[10px] bg-[var(--pc-surface)] py-2.5 px-2 text-center">
							...
							<div class="mt-1 text-[10px] text-[var(--pc-text-faint)] opacity-40">{stat.label}</div>
						</div>
					{/each}
				</div>
				<a href="/launchpad" class="mt-3 block text-center rounded-full bg-[var(--pc-text)] text-[var(--pc-bg)] py-2 text-xs font-medium hover:opacity-88 transition-opacity">View launchpad →</a>
			</div>
				...
				<div class="mt-3 space-y-2.5">
					{#each mockStates.slice(0, 3) as m}
						<a href="/p/{m.product.slug}" class="flex items-center gap-2.5 group">
							<img src={m.maker.avatar} alt={m.maker.name} class="size-7 rounded-full object-cover" />
							<div class="min-w-0 flex-1">
								...
								<div class="text-[11px] text-[var(--pc-text-muted)] opacity-40 line-clamp-1 mt-0.5">{m.product.name}</div>
							</div>
			...
			<div class="rounded-[20px] bg-[var(--pc-surface-2)] p-4">
				...
				<p class="mt-1.5 text-[11px] leading-relaxed text-[var(--pc-text-muted)] opacity-40 text-pretty">
					Every launch gets an SEO-ready page for Google + AI agents.
				</p>
				<div class="mt-2.5 flex flex-wrap gap-1.5">
					{#each ['SEO tools', 'DevTool', 'AI'] as tag}
						<span class="rounded-full bg-[var(--pc-surface)] px-2 py-0.5 text-[11px] text-[var(--pc-text-muted)] opacity-50">{tag}</span>
					{/each}
				</div>
				<a href="/products" class="mt-2.5 inline-flex text-[11px] text-[var(--pc-accent)]">Browse all →</a>
			</div>

			<!-- Streak leaderboard -->
			<div class="rounded-[20px] bg-[var(--pc-surface-2)] p-4">
				<h4 class="text-[11px] font-medium uppercase tracking-wider text-[var(--pc-text-faint)] opacity-40">Streaks</h4>
				<ol class="mt-2.5 space-y-1.5">
					{#each [
						...
						<li class="flex items-center gap-2 text-[13px]">
							<span class="size-[18px] grid place-items-center rounded-full text-[10px] font-medium {i === 0 ? 'bg-[var(--pc-accent)] text-white' : 'bg-[var(--pc-surface)] text-[var(--pc-text-muted)]'}">{i + 1}</span>
							<span class="opacity-70">{item.name}</span>
							<span class="ml-auto text-[var(--pc-text-faint)] opacity-40">{item.score}</span>
						</li>
					{/each}
				</ol>
			</div>
...
<!-- Mobile bottom nav — full width, not constrained -->
