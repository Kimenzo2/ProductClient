<script lang="ts">
	...
	let productOptions = $derived(publicProducts.map((p) => ({ value: p.slug, label: p.name })) );
...
<div class="mx-auto w-full max-w-[760px] px-4 sm:px-6">
	<header class="pb-6 pt-8 sm:pt-12"><a href="/" class="inline-flex items-center gap-1 text-xs text-[var(--pc-text-muted)] hover:text-[var(--pc-text)]"><ArrowLeft size={13} weight="Outline" /> Back to Product Client</a><h1 class="mt-2 text-[26px] font-medium tracking-tight md:text-[34px]">Tell us what you think.</h1><p class="mt-2 max-w-[58ch] text-[14px] md:text-[15px] leading-[1.65] tracking-[-0.009em] text-[var(--pc-text-muted)] max-w-[60ch] text-pretty antialiased">Tell the team what worked, what went wrong, or what would make the product better.</p></header>
	{#if submitted}
		<Card padding="lg" class="mb-12 text-center"><div class="mx-auto grid size-11 place-items-center rounded-full bg-[var(--pc-accent)]/15 text-[var(--pc-accent-light)]"><CheckCircle size={22} weight="Outline" /></div><h2 class="mt-4 text-lg font-medium">Feedback received</h2><p class="mx-auto mt-2 max-w-[44ch] text-[14px] md:text-[15px] leading-[1.65] tracking-[-0.009em] text-[var(--pc-text-muted)] max-w-[60ch] text-pretty antialiased">The {publicProducts.find((product) => product.slug === productSlug)?.name ?? 'product'} team can now review your message and decide what to do next.</p><Button href="/" variant="primary" class="mt-5">Explore products</Button></Card>
	{:else}
		<form onsubmit={(event) => { event.preventDefault(); submit(); }} class="mb-12 space-y-5">
			<Card padding="lg">
				<div class="grid gap-5 sm:grid-cols-2">
					<div><Label for="feedback-product">Product</Label><div class="mt-2"><Select id="feedback-product" bind:value={productSlug} options={productOptions} placeholder="Select a product" /></div></div>
