...
- **Feature:** Hover on a heavy workspace icon (Docs, Incidents, etc.) in the primary rail (60px collapsed / 240px expanded at `src/lib/components/layout/Sidebar.svelte:1`) expands an internal secondary panel (240px, flat, opaque). Pinned when on that route, preview on hover otherwise. Mimics Supabase project nav: narrow rail + second column.
