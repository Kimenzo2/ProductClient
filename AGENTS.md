...
- **UnoCSS requires `per-module` mode** — SvelteKit 3 adopted the Vite Environment API which does not support `transformIndexHtml`. UnoCSS global mode (the default) relies on this hook to inject CSS during builds. When it fails, all atomic CSS utilities silently vanish from the production bundle — dev works perfectly, production is completely unstyled. The fix is `mode: 'per-module'` in `vite.config.ts`. Also requires `@unocss/extractor-svelte` for Svelte template parsing. Do not remove either. References: https://unocss.dev/integrations/vite#sveltekit https://github.com/unocss/unocss/issues/4990
